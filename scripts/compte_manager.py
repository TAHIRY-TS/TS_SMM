...
CONFIG1_PATH = os.path.join(PROJECT_ROOT, "config", "config1.json")
...
def tester_session(username):
    ...
    try:
        ...
        api = Client(username, "dummy", settings=session)
        info(f"Session valide pour {username}.")
    except Exception as e:
...
def supprimer_session(username):
    ...
    if os.path.exists(path):
        os.remove(path)
        success(f"Session de {username} supprimée.")
    else:
        erreur(f"Aucune session trouvée pour {username}.")
...
def menu():
    while True:
        ...
        print("4. Tester une session")
        print("5. Supprimer une session")
        print("0. Quitter")
        print()
